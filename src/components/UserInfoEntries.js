...
export default function UserInfoEntries() {
    ...
    const saveResume = async () => {
        ...
        console.log(u)
        setSaved(false);
        axios.get("http://localhost:4000/api/data/getAllDetailsByEmail", {
            headers: {
                email: user.email
            }
        }).then((res)=>{
            ...
            if (result.length>0) {
                ...
                axios.post("http://localhost:4000/api/data/addDetails", body).then((res)=>{
                    setSaved(true);
    ...
    const saveAndOverwrite = async () => {
        ...
        axios.get("http://localhost:4000/api/data/getAllDetailsByEmail", {
            ...
            if (result.length>0) {
                ...
                axios.post("http://localhost:4000/api/data/updateDetailsByDetailId",body).then((res)=>{
                }).catch((error)=>{
    ...
    return (
        <>
            ...
            <div className="flex my-5 bg-white pt-[10vh] w-full p-8 h-[100vh]">
                                    ...
                                    <div className="">
                                        <div className="w-full ">
                                            <div
                                                ...
                                                ref={completed}
                                            ></div>
                                            <div className="font-manrope text-center text-3xl">
                ...
                <Modal open={open} onClose={() => {setSaved(false); setOpen(false)}}>
                    {/* {setSaved(false)} */}
                    <ModalDialog>
                        ...
                        <DialogContent>
                            ...
                            <Stack spacing={2}>
                                <FormControl error={error!==""}>
                                    ...
                                    <Input  value={resumeName} onChange={(e)=>{setError("");setResumeName(e.target.value)}} autoFocus required />
                                    <FormHelperText > {error}</FormHelperText>
                                </FormControl>
                                <ButtonGroup  className="" orientation="vertical">
                                <ButtonJoy variant="outlined" className="w-full" color={saved?"success":"primary"} type="submit" onClick={saveResume}>{saved?"Saved Successfully!":"Save"}</ButtonJoy>
                                <ButtonJoy variant="outlined" color="primary" className="w-full" onClick={saveAndOverwrite}>Save and Overwrite</ButtonJoy>
                                </ButtonGroup>
                            </Stack>
                    </ModalDialog>
                </Modal>
